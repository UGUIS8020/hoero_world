document.addEventListener("DOMContentLoaded", function () {
    ...
    async function uploadFiles(files) {
        ...
        const selectedTeeth = [];
        document
            .querySelectorAll('input[name="teeth[]"]:checked')
            .forEach((cb) => {
        ...
        // フォルダ構造の有無を判定
        const hasFolder = Object.values(paths).some((path) =>
            path.includes("/")
        );
    ...
    uploadButton.addEventListener("click", () => {
        ...
        if (!message) {
            ...
            alert(
                "メッセージが未入力です。入力してからアップロードしてください。"
            );
            return; // 処理を中止
        }
    ...
    document.addEventListener("DOMContentLoaded", function () {
        const teethCheckboxes = document.querySelectorAll(
            'input[name="teeth[]"]'
        );
